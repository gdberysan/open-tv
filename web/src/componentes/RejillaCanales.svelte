<script lang="ts">
  ...
  // de la lista: cuando entra en el viewport, pide la página siguiente. App
  // decide si hay más que pedir (soloFavoritos no pagina, ver Tarea 12).
  $effect(() => {
    if (!centinela) return
...
{:else}
  <div class="canales" class:rejilla={vista === 'rejilla'} class:lista={vista === 'lista'}>
...
<style>
  .canales.rejilla { display: grid; grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap: 12px; }
