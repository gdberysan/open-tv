...
- [ ] **Step 4:** Crear la denylist privada `~/.config/korven/open-tv-denylist.txt` con: `<email-no-canónico>`, `<usuario>` (usuario del Mac), y cualquier teléfono/ruta privada conocida. UNA línea por término. (Este fichero NO se versiona.)
...
- [ ] **Step 3:** Crear el fichero `mailmap` con:
  ```
  Gerard <gdberysan@gmail.com> <<email-no-canónico>>
  Gerard <gdberysan@gmail.com> Gerard Bernal <gdberysan@gmail.com>
  ```
  (dependabot[bot] NO se toca.)
- [ ] **Step 4:** Correr `git filter-repo --mailmap mailmap --force` sobre el repo. Verificar con `git shortlog -sne --all` que solo quedan `Gerard <gdberysan@gmail.com>` y `dependabot[bot]` (0 `el email no-canónico`, 0 `Gerard Bernal`).
